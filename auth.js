/* auth.js (r5) - inicialização segura: espera MSAL antes de iniciar */
(function(){
  ...
  function init(){ 
    ...
    const msalConfig = {
      auth: { clientId: CLIENT_ID, authority: "https://login.microsoftonline.com/" + TENANT_ID,
              redirectUri: REDIRECT_URI, postLogoutRedirectUri: REDIRECT_URI, navigateToLoginRequestUrl: false },
      cache: { cacheLocation: "localStorage", storeAuthStateInCookie: true },
      system: { loggerOptions: { loggerCallback: function(){}, piiLoggingEnabled: false } }
    };
    ...
    async function handleRedirect(){ try{ await msalInstance.handleRedirectPromise(); } catch(e){ console.error("MSAL redirect error:", e); } }
    ...
    async function selectOrLogin(){ 
      ...
      if (account){ 
        ...
        if (pending && /\/login\.html$/i.test(location.pathname)){ 
          const url = atob(pending); sessionStorage.removeItem("postLoginPath"); location.replace(url || "/index.html"); 
        }
        return account; 
      }
      return null; 
    ...
    async function beginLogin(){ 
      ...
      if (!acc){ try{ await msalInstance.loginRedirect(loginRequest); } catch(e){ console.error("Erro no loginRedirect:", e); } } 
    }

    function logout(){ try{ msalInstance.logoutRedirect(); } catch(e){ console.error(e); } }
    ...
    async function getToken(scopes){ 
      ...
      try{ const r = await msalInstance.acquireTokenSilent(req); return r.accessToken; } 
        ...
        if (e instanceof msal.InteractionRequiredAuthError){ await msalInstance.acquireTokenRedirect(req); return null; }
        throw e; 
      } 
    }

    window.TalentosAuth = { requireAuthGuard, beginLogin, logout, getToken, msalInstance };
  }
  init();
})();
